(function () {
    ...
    fb.simplelogin.util.misc.parseQuerystring = function (a) {
        for (var b = {}, a = a.replace(/^\?/, "").split("&"), c = 0; c < a.length; c++)
            if (a[c]) {
                var d = a[c].split("=");
                b[d[0]] = d[1]
            }
        ...
        return fb.simplelogin.util.json && fb.simplelogin.util.json.eval && fb.simplelogin.util.json.stringify && window.postMessage ? {
            open: function (f, g, h) {
                ...
                var w = setInterval(function () {
                    ...
                    x = fb.simplelogin.util.json.stringify({
                        a: "request",
                        d: g.params
                    });
                a(window, "unload", m);
    ...
    fb.simplelogin.transports.TriggerIoTab_.prototype.open = function (a, b, c) {
        ...
        forge.tabs.openWithOptions({
            ...
            var b;
            if (a && a.url) try {
                var c = fb.simplelogin.util.misc.parseUrl(a.url),
                    h = fb.simplelogin.util.misc.parseQuerystring(decodeURIComponent(c.hash)),
                    a = {},
                    i;
                for (i in h) a[i] =
        ...
        for (var c = 0, d = goog.string.trim(String(a)).split("."), e = goog.string.trim(String(b)).split("."), f = Math.max(d.length, e.length), g = 0; 0 == c && g < f; g++) {
            ...
            do {
                ...
                var c = 0 == m[1].length ? 0 : parseInt(m[1], 10),
                    ...
                    c = goog.string.compareElements_(c, o) || goog.string.compareElements_(0 ==
    ...
    goog.net.XhrIo.prototype.onReadyStateChangeHelper_ = function () {
        if (this.active_ && "undefined" != typeof goog)
            if (this.xhrOptions_[goog.net.XmlHttp.OptionType.LOCAL_REQUEST_ERROR] && this.getReadyState() == goog.net.XmlHttp.ReadyState.COMPLETE && 2 == this.getStatus()) this.logger_.fine(this.formatMsg_("Local request error detected and ignored"));
    ...
    fb.simplelogin.transports.XHR_.prototype.open = function (a, b, c) {
        var d = this,
            e = new goog.net.XhrIo;
        goog.events.listen(e, "complete", function () {
            if (e.isSuccess()) {
                var a = e.getResponseJson(),
                    b = a.error || null;
                delete a.error;
                c && c(b, a)
            } else c && c(d.formatError_({
                code: "SERVER_ERROR",
    ...
    fb.simplelogin.util.validation.validateContextObject = function (a, b, c, d) {
        if (!d || goog.isDef(c))
            if (!goog.isObject(c) || null === c) throw Error(fb.simplelogin.util.validation.errorPrefix_(a, b, d) + "must be a valid context object.");
    };
    ...
    fb.simplelogin.transports.WindowsMetroAuthBroker_.prototype.open = function (a, b, c) {
        ...
        g(e.none, a, d).done(function (a) {
                ...
                if (a && a.responseData) try {
                    var c = fb.simplelogin.util.misc.parseUrl(a.responseData),
                        d = fb.simplelogin.util.misc.parseQuerystring(decodeURIComponent(c.hash)),
                        a = {},
                        e;
                    for (e in d) a[e] = fb.simplelogin.util.json.eval(d[e]);
                    b = a
                } catch (f) {}
                b && b.token && b.user ? i(null, b) : b && b.error ? i(b.error) : i({
                    code: "UNKNOWN_ERROR",
                    message: "An unknown error occurred."
                })
            },
    ...
    var cookieStoragePath = "/",
        encryptionStorageKey = "firebaseSessionKey",
        sessionPersistentStorageKey = "firebaseSession",
        hasLocalStorage = fb.simplelogin.util.env.hasLocalStorage();
    fb.simplelogin.SessionStore_ = function () {};
    fb.simplelogin.SessionStore_.prototype.set = function (a, b) {
        if (hasLocalStorage) try {
            var c = a.sessionKey,
                d = sjcl.encrypt(c, fb.simplelogin.util.json.stringify(a));
        ...
        if (hasLocalStorage) {
            try {
                var a = goog.net.cookies.get(encryptionStorageKey),
                    b = localStorage.getItem(sessionPersistentStorageKey);
    ...
    fb.simplelogin.client.prototype.resumeSession = function () {
        ...
        if (b) {
            ...
            d.open(fb.simplelogin.Vars.getApiHost() + "/auth/session", {
                ...
                if (c && c.token && c.user) a.attemptAuth(c.token, c.user, true);
                else if (b) {
    ...
    fb.simplelogin.client.prototype.login = function () {
        ...
        var a = arguments[1] || {},
            b = arguments[0].toLowerCase(),
            c = "FirebaseSimpleLogin.login(" + b + ")";
        if ("password" === b) {
    ...
    fb.simplelogin.client.prototype.loginViaToken = function (a, b) {
        var b = b || {},
            c = this,
            d = fb.simplelogin.Vars.getApiHost() + "/auth/" + a + "/token?firebase=" + c.mNamespace;
    ...
    fb.simplelogin.client.prototype.loginViaOAuth = function (a, b) {
        var b = b || {},
            c = this,
            d = fb.simplelogin.Vars.getApiHost() + "/auth/" + a + "?firebase=" + this.mNamespace;
        ...
        var f = fb.simplelogin.util.env.isMobileCordovaInAppBrowser() ? "mobile-phonegap" : fb.simplelogin.util.env.isMobileTriggerIoTab() ?
            "mobile-triggerio" : fb.simplelogin.util.env.isWindowsMetro() ? "windows-metro" : "desktop",
            g;
        if ("desktop" === f) {
            g = "WinChan";
            var f = [],
        ...
        } else "mobile-phonegap" === f ? g = "CordovaInAppBrowser" : "mobile-triggerio" === f ? g = "TriggerIoTab" : "windows-metro" === f && (g = "WindowsMetroAuthBroker"); if (b.preferRedirect || fb.simplelogin.util.env.isChromeiOS() || fb.simplelogin.util.env.isWindowsPhone() ||
        ...
        fb.simplelogin.transports[g].open(d, b, function (a, b) {
            if (b && b.token && b.user) c.attemptAuth(b.token, b.user, true);
            else {
                var d = a || {
                    code: "UNKNOWN_ERROR",
                    message: "An unknown error occurred."
                };
                if (a === "unknown closed window") d = {
                    code: "USER_DENIED",
                    message: "User cancelled the authentication request."
                };
                else if (b && b.error) d = b.error;
